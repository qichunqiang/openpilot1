...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    sp_tss2_long_cydia_tune = Params().get_bool("ToyotaTSS2LongCydia")
    ...
    # Apply tuning based on conditions
    if sp_tss2_long_tune or ret.enableGasInterceptorDEPRECATED:
      tune.deadzoneBP = [0., 4., 5., 6., 20., 30]
      tune.deadzoneV = [0., 0., 0.001, 0.003, 0.1, 0.15]

      if candidate in TSS2_CAR:
        tune.kiBP = [0., 0.1, 1., 2., 3., 5., 8., 12., 20., 27., 36., 50]
        tune.kiV = [0.348, 0.351, 0.33, 0.3085, 0.286, 0.227, 0.221, 0.20, 0.17, 0.10, 0.10, 0.01]
        apply_tss2_car_tuning()
      else:
        apply_default_tuning()

    elif sp_tss2_long_cydia_tune:
      ret.stopAccel = -2.5
    ...
    elif candidate in TSS2_CAR or ret.enableGasInterceptorDEPRECATED:
      tune.deadzoneBP = [0., 9.]
      tune.deadzoneV = [0., 0.15]
      tune.kpBP = [0., 5., 20.]
      tune.kpV = [1.3, 1.0, 0.7]
      tune.kiBP = [0., 5., 12., 20., 27.]
      tune.kiV = [0.35, 0.23, 0.20, 0.17, 0.1]
