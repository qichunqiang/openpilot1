...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.stock_aeb = {}
    self._brakehold_time = 100 * 1   # 1 seconds stopped to activate
    self._brakehold_active = False
    self._brakehold_counter = 0
    self._reset_brakehold = False
    self._prev_brakePressed = True
    self._brakehold = False
  ...
  def update(self, cp, cp_cam):
    ...
    if self.sp_toyota_enhanced_bsm and self.frame > 1999:
    ...
    if self.sp_toyota_auto_hold:
      ...
      self._brakehold_active =  ret.standstill and ret.cruiseState.available and not ret.gasPressed and \
                                   not ret.cruiseState.enabled and (ret.gearShifter not in (self.GearShifter.reverse, \
                                                                                            self.GearShifter.park))
      if self._brakehold_active:
        self._brakehold_counter += 1
        self._brakehold = (self._brakehold_counter > self._brakehold_time and
                              not self._reset_brakehold)
        self._reset_brakehold = (not self._prev_brakePressed and ret.brakePressed and not self._reset_brakehold)
      else:
        self._brakehold = False
        self._reset_brakehold = False
        self._brakehold_counter = 0
      self._prev_brakePressed = ret.brakePressed
  ...
  def get_can_parser(CP):
    ...
    params = Params()
    if params.get_bool('sp_toyota_enhanced_bsm'):
